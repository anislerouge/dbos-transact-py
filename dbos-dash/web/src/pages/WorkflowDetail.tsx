...
export default function WorkflowDetail() {
  ...
  useEffect(() => {
    ...
    api.getWorkflow(workflowId).then(setWorkflow).catch(e => setError(String(e)))
    api.getSteps(workflowId).then(setSteps).catch(console.error)
  }, [workflowId])
  ...
  const handleRestart = async () => {
    if (!workflow?.WorkflowName) return
    ...
    try {
      ...
      const result = await api.startWorkflow(workflow.WorkflowName, args, kwargs)
